...
class net(nn.Module):
    def __init__(self, M, n, z_dimension):
        ...
        self.o = nn.Linear(int(z_dimension*z_dimension/4)+n, n)
        self.r = nn.Sequential(
            nn.Linear(n, 32),
            nn.ReLU(),
            nn.Linear(32, M)
    ...
    def forward(self, x):
        temp = x
        noise = temp[:,0:z_dimension*z_dimension]     #input batch_size*(M+z_dimension)
        x = temp[:,z_dimension*z_dimension:]
        x = self.t(x)
        x = nn.functional.normalize(x, 2, 1)*np.sqrt(n)
        noise = noise.view(-1, 1, z_dimension, z_dimension)
        noise = self.g(noise)
...
class generator_CNN(nn.Module):
    ...
    def forward(self, x):
        temp = x
        x = temp[:,0:z_dimension*z_dimension]
        coding = temp[:,z_dimension*z_dimension:]
        x = x.view(-1 ,1 , z_dimension, z_dimension)
        x = self.g(x)
        x = x.view(x.size(0), -1)
        x = torch.cat([x, coding],1)
        x = self.o(x)
        return x
...
G = generator_CNN(M, n, z_dimension)
D = discriminator(M, n)
...
for epoch in range(num_epoch):
    ...
    noise = np.random.randn(batch_size,z_dimension*z_dimension)
    ...
    for name, value in NT.named_parameters():
        value.requires_grad = True
        if name[0] == 'r' or name[0] == 'g' or name[0] == 'o':
            value.requires_grad = False
    # params_t = filter(lambda p: p.requires_grad, NT.parameters())
    # t_optimizer = torch.optim.Adam(params_t, lr=LR_t)
    ...
    for i in range(num_GAN):
        ...
        # loss of fake data
        z_noise = torch.Tensor(np.random.randn(batch_size, z_dimension*z_dimension))
    ...
    for name, value in NT.named_parameters():
        value.requires_grad = True
        if name[0] == 't' or name[0] == 'g' or name[0] == 'o':
            value.requires_grad = False
    # params_r = filter(lambda p: p.requires_grad, NT.parameters())
    # r_optimizer = torch.optim.Adam(params_r, lr=LR_r)
    #begin training
    ...
    if epoch % 100 == 0:
        ...
        x_test_1 = x_test[:,:]
        noise = np.random.randn(num_test,z_dimension*z_dimension)
...
torch.save(NT.state_dict(),'./BER/SNR-7-0.0002')
sio.savemat('GAN_7_0.0002 _%d%d'%(n,k),{'SNR_dB':SNR_list, 'ber':ber_result})